...
```{r}
#Create an empty Date dataframe
all_date <- seq(as.Date("2020/8/1"), as.Date("2021/1/1"), by = 'day')
zero <- seq(0,0,length.out=154)
df <- data.frame(opendategrouped = all_date, zero = zero)
enrol_district <- OH_K12 %>%
  distinct(districtname, district_enroll, opendategrouped, teachingmethod) %>%
  group_by(opendategrouped, teachingmethod) %>%
  summarise(district_enroll = sum(district_enroll),.groups = 'drop') %>%
  filter(teachingmethod %in% c('On Premises','Hybrid'))
```

```{r}
enrol_district <- left_join(df, enrol_district, by = 'opendategrouped')
```

```{r}
enrol_district[is.na(enrol_district)] <- 0
```
...
```{r}
# Plot
enrol_district<-enrol_district[,-2]
enrol_district<-enrol_district%>%filter(teachingmethod!=0)
enrol_district$opendategrouped<-as.Date(enrol_district$opendategrouped)

ggplot() +
  geom_bar(data=enrol_district,aes(y=district_enroll, x=opendategrouped,  fill=teachingmethod),position = 'stack', stat = 'identity') +
  geom_line(data=ohio_age%>%filter(date>=as.Date("2020-08-01")&date<=as.Date("2021-01-01")),aes(x=date,y=case_19_per*12000,col="Age 0-19 Percentage"))+
  scale_y_continuous(
    sec.axis = sec_axis( trans=~./12000, name="Age 0-19 Percentage")
  )+
  geom_rect(data = enrol_district[1,],
            aes(xmin = as.Date('2020/08/01'), 
                xmax = as.Date('2020/12/12'), ymin=-Inf, ymax=Inf), 
            color = NA, alpha = 0.2, show.legend = F, fill = 'orange') +
    xlab('School Reopen Date (grouped by District)') +
    ylab('Student Enrollment') +
  ggtitle('Number of Student Enrollment on Different School Reopen Dates \n(On Premises & Hybrid)') +labs(col="")+
