...
na.omit(B0B1)%>%
  ...
  geom_boxplot(aes(fill=major_teaching))+ylim(c(0,0.05))+
  ...
  team_theme+theme(legend.position = "bottom")+
  labs(x="Max B",y="Urban-Rural Status",title="Distribution of Max B in Micropolitan Counties",
       fill="Majority Teaching Method")+
  scale_color_manual(values=col_theme)+
  grid_theme+theme(legend.text = element_text(size=12),
                   legend.title = element_text(size=12),
                   axis.text = element_text(size=11),
                   title=element_text(size=14),strip.text.x = element_text(
        size = 10, face = "bold.italic"
        ))+
  scale_fill_manual(values=col_theme)

ggsave("maxB_micro.png",width = 6,height = 6)
```
...
B_diff%>%
  ...
  labs(y="B(6w) - B(3w)",x="B(3w) - B(0w)",
       title="Acceleration before vs. after school reopen\nAll Counties",
       color="Majority Teaching Method",
       size = "Log of Population Density",
       alpha= "Log of Population Density" ,fill="Majority Teaching Method")+
...
```{r}
B_diff_micro%>%
  ggplot(aes(x=log(Population.density),y=new.slope.diff,group=major_teaching,color=major_teaching))+geom_point(aes(size=avg_full_work_prob,alpha=avg_full_work_prob))+geom_smooth(method = "lm", se=F, formula = y ~ x,alpha=0.1)+team_theme+
  labs(y="Acceleration",x="Log of Population Density",
       title="Difference in slope of B(0w) and B(3w)\nOnly Micropolitan Counties",
       color="Majority Teaching Method",
       size = "0w~3w\nAveraged %6hr+ Away Home",
       alpha= "0w~3w\nAveraged %6hr+ Away Home" ,fill="Majority Teaching Method")+scale_color_manual(values=col_theme)+theme(legend.position = "bottom")+geom_text(data = onpremise_text,aes(label=County),color='black',size=4,hjust=0.6, vjust=-0.8)

ggsave("acc_pop_micro.png",width = 7.5,height = 6)


B_diff_micro%>%
  ggplot(aes(x=log(Population.density),y=new.slope.diff2,group=major_teaching,color=major_teaching))+geom_point(aes(size=avg2_full_work_prob,alpha=avg2_full_work_prob))+geom_smooth(method = "lm", se=F, formula = y ~ x,alpha=0.1)+team_theme+
  labs(y="Acceleration",x="Log of Population Density",
       title="Difference in slope of B(3w) and B(6w)\nOnly Micropolitan Counties",
       color="Majority Teaching Method",
       size = "3w~6w\nAveraged %6hr+ Away Home",
       alpha= "3w~6w\nAveraged %6hr+ Away Home" ,fill="Majority Teaching Method")+
  scale_color_manual(values=col_theme)+theme(legend.position = "bottom")+
  geom_text(data=onpremise_text,aes(label=County),color='black',size=4,hjust=0.6, vjust=-0.1)
...
teaching_profile%>%
  ...
  labs(title="Percent of 65years+ Population by Teaching Method",x="Major Teaching Method",y="Percent of 65+yrs population",fill="Majority Teaching Method",caption = "Post-hoc pairwise testing with Duncan Method")+
...
ggplot(gamma_plot)+
  geom_line(aes(x=time1,y=prob,colour = "At day 0"))+
  geom_line(aes(x=time2,y=prob,colour = "At day 5"))+
  geom_line(aes(x=time3,y=prob,colour = "At day 10"))+
  geom_line(aes(x=time4,y=prob,colour = "At day 15"))+
  geom_vline(xintercept = 25.2,lty=2,colour="darkgreen")+
  labs(x="Time from infections to deaths",
       y="Probability of died after x days",
       colour="Day of Infections")+team_theme+scale_color_brewer(palette = 6,type="div")+theme(legend.position = "bottom")

ggsave("gamma.png",width = 5,height = 3)
```
