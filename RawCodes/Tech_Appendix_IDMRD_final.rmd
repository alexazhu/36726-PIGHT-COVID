...
```{r}
table(wide_teaching_enroll$major_teaching)
```
...
wide_teaching_enroll%>%
  ...
  theme(legend.position = "right",legend.text = element_text(size=19),legend.title = element_text(size=20))

ggsave("Online-only map.png",height = 5,width = 5.5)
...
ggsave("On-premises map.png",height = 5,width = 5.5)
...
ggsave("Hybrid.png",height = 5,width = 5.5)
...
ggsave("Majority Teaching Map.png",height = 5,width = 5)
```
...
ggplot(fall_cases,aes(y = death_incidence_per_1000, x = major_teaching)) + 
  ...
  stat_pvalue_manual(stat.test, label = "p",y.position = 2.5, step.increase = 0.15,size=6)+
  ylim(c(0,3.5))+ 
  theme_bw()+ 
...
ggsave("Death incidences plot_boxplots.png",width = 8,height = 6.5)
```
...
case_policy_wide%>%
  ...
  labs(x = "Date", y = "Cumulative Death Incidence / 1,000 people",
       subtitle = "Yellow area represents fall semester (09/18/20 - 12/15/20)",
       color = "Majority teaching posture") +team_theme+ scale_colour_manual(values=col_theme)+scale_fill_manual(values=col_theme)
ggsave("Death incidences plot_three lines.png",width = 8,height = 6.5)
```
...
teaching_profile%>%
  ...
  labs(title="Percent of Uninsured Population",x="Majority teaching posture",y="Percent of uninsured population",caption = "Pairwise p-values come from Duncan pairwise comparison test")+
  team_theme+theme(legend.position = "")+scale_fill_manual(values=col_theme)

ggsave("uninsure population_box plots.png",width = 7,height = 6)
```
...
teaching_profile%>%
  ...
  stat_pvalue_manual(stat.test, label = "p",y.position = 23, step.increase = 0.15,size = 5,bracket.nudge.y = 2)+
  labs(title="Percent of Population over 65-year-old",
       x="Major Teaching  Posture",y="Percent of 65+yrs population",fill="Majority teaching posture",caption = "Pairwise p-values come from Duncan pairwise comparison test")+team_theme+theme(legend.position = "")+scale_fill_manual(values=col_theme)

ggsave("senior population_box plots.png",width = 7,height = 6)
...
ggsave("Growth coefs plot_three lines.png",width = 8,height = 6.5)
```
...
na.omit(B0B1)%>%
  ...
  stat_compare_means(method = "anova",size=6,label.y.npc=1)+ 
  stat_pvalue_manual(stat.test, label = "p",y.position = 0.04, step.increase = 0.15,size = 6)+
...
ggsave("Max growths_box plots.png",width = 8,height = 6.5)
```
...
na.omit(B0B1)%>%
  ...
  labs(y="Maximum Growth Coefficient",x="Averaged \n%6hr+ Away Home in Fall semester",title="Maximum Growth Coefficient in Fall v.s. Average Mobility \nAll Counties",color="Majority teaching posture",size = "Log of Population Density",alpha= "Log of Population Density" )+
  team_theme+theme(legend.position = "bottom")+
  scale_color_manual(values=col_theme)

ggsave("Max growths_avg mobility.png",width = 7.6,height = 6.8)
...
na.omit(B0B1)%>%
  ...
  geom_smooth(method = "lm", se=F, formula = y ~ x,alpha=0.1)+
  labs(y="Maximum Growth Coefficient",x="Log of Population Density",title="Maximum Growth Coefficient in Fall v.s. Log of Population Density \nAll Counties",color="Majority teaching posture",size = "Averaged \n%6hr+ Away Home in Fall semester",alpha= "Mobility in 08/18/20 - 09/09/20" )+team_theme+theme(legend.position = "bottom")+
  scale_color_manual(values=col_theme)

ggsave("Max growths_log pop.png",width =8.8,height = 7.5)
```
...
```{r}
micro_counties <- ohio_profile%>%
  left_join(wide_teaching_enroll,
            by = c("County" = "county"))%>%
  drop_na(major_teaching)%>%
  filter(NCHS.Urban.Rural.Status=="Micropolitan")%>%
  select(County,Hybrid,On_Premises,Online_Only)

write.csv(micro_counties,"Micropolitan Counties Teaching Profile.csv")
```
...
ggsave("log pop_rural_boxplots.png",width = 10,height = 6)
```
...
ggsave("mirco_log pop_teach_violin.png",width = 7,height = 6)
```
...
ggsave("Micro_majority teaching map.png",width = 12,height = 12)
```
...
na.omit(B0B1)%>%
  ...
  stat_pvalue_manual(stat.test, label = "p",y.position = 0.03, step.increase = 0.15,size = 5)+
  team_theme+
  theme(legend.position = " ")+
...
ggsave("Micro_Max growths_box plots.png",width = 7,height = 7)
```
...
## 6.6 Change in Growth vs. mobility and population density for Micropolitan counties
...
ggsave("micro_change in growth_right after school reopen.png",width = 7.3,height = 6.5)
...
B_diff_micro%>%
  ...
  labs(y="Change in Growth",x="Log of Population Density",
       ...
       size = "3w~6w\nAveraged %6hr+ Away Home",
       alpha= "3w~6w\nAveraged %6hr+ Away Home" ,fill="Majority teaching posture")+scale_color_manual(values=col_theme)+theme(legend.position = "bottom")+geom_text(data =B_diff_micro%>%filter(major_teaching=="On Premises"),aes(label=COUNTY),color='black',size=3,hjust=1.1, vjust=0.3)
ggsave("micro_change in growth_3wk after school reopen.png",width = 7.3,height = 6.5)
```
