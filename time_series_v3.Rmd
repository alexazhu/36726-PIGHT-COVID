...
case_mobility%>%
  ...
  mutate(death_prop = CUMDEATHS/POPULATION)%>%
  ggplot(aes(x=DATE,y=res_visit_prop))+geom_point()+geom_smooth()+labs(x="",y="Restaurant Visits")
...
avg_work%>%
  ggplot(aes(x = DATE, y = avg_ptwork_prop_7d, color = major_teaching))+
    geom_rect(data = avg_work[1,],
            aes(xmin=as.Date("2020/08/26"), xmax=as.Date("2020/12/12")),
                ymin=-Inf,ymax=Inf,color = NA,alpha=0.3,show.legend = F, fill = "orange") +  
    geom_line(aes(color = major_teaching),size = 1, alpha = .8) + 
        geom_vline(xintercept = date.intercept, linetype = "dashed") + 
...
Considering the lagging deaths, if we want to know the part-time work effects on the deaths, we should fit 

y1 death after 3 weeks ~ y0 death today + work_prop_7d today

Check y0_label and y1_label for what window it is

```{r}
y1y0 <- start_of_window %>%
  rename(y0_label = y_label)%>%
  group_by(COUNTY)%>%
  arrange(DATE)%>%
  mutate(y0= death_prop,y1 = lead(y0,n=1))%>%
  mutate(y1_label = lead(y0_label,n=1))%>%
  drop_na(y1)

summary(lm(y1~y0+work_prop_7d,na.action='na.omit',data = y1y0))
summary(lm(y1~y0+res_visit_prop,na.action='na.omit',data = y1y0))
summary(lm(y1~y0+bar_visit_prop,na.action='na.omit',data = y1y0))
```
...
```{r}
summary(lm(y1~y0+work_prop_7d+On_Premises,na.action='na.omit',data = y1y0))
```
